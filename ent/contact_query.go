...
func (cq *ContactQuery) QueryContact() *UserQuery {
	query := (&UserClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (cq *ContactQuery) Clone() *ContactQuery {
	...
	return &ContactQuery{
		...
		withContact: cq.withContact.Clone(),
...
func (cq *ContactQuery) WithContact(opts ...func(*UserQuery)) *ContactQuery {
	query := (&UserClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withContact = query
	return cq
}
...
//		GroupBy(contact.FieldIsFavourite).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Contact.Query().
//		Select(contact.FieldIsFavourite).
//		Scan(ctx, &v)
...
func (cq *ContactQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Contact, error) {
	...
	if cq.withOwner != nil || cq.withContact != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := cq.withContact; query != nil {
		if err := cq.loadContact(ctx, query, nodes, nil,
			func(n *Contact, e *User) { n.Edges.Contact = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *ContactQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*Contact, init func(*Contact), assign func(*Contact, *User)) error {
	...
	for i := range nodes {
		if nodes[i].contact_owner == nil {
			continue
		}
		fk := *nodes[i].contact_owner
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "contact_owner" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (cq *ContactQuery) loadContact(ctx context.Context, query *UserQuery, nodes []*Contact, init func(*Contact), assign func(*Contact, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Contact)
	for i := range nodes {
		if nodes[i].contact_contact == nil {
			continue
		}
		fk := *nodes[i].contact_contact
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "contact_contact" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
