...
func (nq *NotificationQuery) QueryRoom() *RoomQuery {
	query := (&RoomClient{config: nq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := nq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (nq *NotificationQuery) QueryMessage() *MessageQuery {
	query := (&MessageClient{config: nq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := nq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (nq *NotificationQuery) Clone() *NotificationQuery {
	...
	return &NotificationQuery{
		...
		withRoom:      nq.withRoom.Clone(),
		withMessage:   nq.withMessage.Clone(),
...
func (nq *NotificationQuery) WithRoom(opts ...func(*RoomQuery)) *NotificationQuery {
	query := (&RoomClient{config: nq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (nq *NotificationQuery) WithMessage(opts ...func(*MessageQuery)) *NotificationQuery {
	query := (&MessageClient{config: nq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	nq.withMessage = query
	return nq
}
...
//		GroupBy(notification.FieldKind).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (nq *NotificationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Notification, error) {
	...
	if nq.withRecipient != nil || nq.withRoom != nil || nq.withMessage != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := nq.withRoom; query != nil {
		if err := nq.loadRoom(ctx, query, nodes, nil,
			func(n *Notification, e *Room) { n.Edges.Room = e }); err != nil {
			return nil, err
		}
	}
	if query := nq.withMessage; query != nil {
		if err := nq.loadMessage(ctx, query, nodes, nil,
			func(n *Notification, e *Message) { n.Edges.Message = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (nq *NotificationQuery) loadRoom(ctx context.Context, query *RoomQuery, nodes []*Notification, init func(*Notification), assign func(*Notification, *Room)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Notification)
	for i := range nodes {
		if nodes[i].notification_room == nil {
			continue
		}
		fk := *nodes[i].notification_room
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "notification_room" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (nq *NotificationQuery) loadMessage(ctx context.Context, query *MessageQuery, nodes []*Notification, init func(*Notification), assign func(*Notification, *Message)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Notification)
	for i := range nodes {
		if nodes[i].notification_message == nil {
			continue
		}
		fk := *nodes[i].notification_message
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "notification_message" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
