...
func (uc *UserCreate) AddMemberships(r ...*RoomMembership) *UserCreate {
	ids := make([]int, len(r))
	for i := range r {
		ids[i] = r[i].ID
	}
...
func (uc *UserCreate) AddUploadedMediumIDs(ids ...int) *UserCreate {
	uc.mutation.AddUploadedMediumIDs(ids...)
	return uc
}

// AddUploadedMedia adds the "uploaded_media" edges to the Media entity.
func (uc *UserCreate) AddUploadedMedia(m ...*Media) *UserCreate {
	ids := make([]int, len(m))
	for i := range m {
		ids[i] = m[i].ID
	}
	return uc.AddUploadedMediumIDs(ids...)
}

// AddOwnedRoomIDs adds the "owned_rooms" edge to the Room entity by IDs.
func (uc *UserCreate) AddOwnedRoomIDs(ids ...int) *UserCreate {
	uc.mutation.AddOwnedRoomIDs(ids...)
	return uc
}

// AddOwnedRooms adds the "owned_rooms" edges to the Room entity.
func (uc *UserCreate) AddOwnedRooms(r ...*Room) *UserCreate {
	ids := make([]int, len(r))
	for i := range r {
		ids[i] = r[i].ID
	}
	return uc.AddOwnedRoomIDs(ids...)
}
...
func (uc *UserCreate) AddContactEntryIDs(ids ...int) *UserCreate {
	uc.mutation.AddContactEntryIDs(ids...)
	return uc
}

// AddContactEntries adds the "contact_entries" edges to the Contact entity.
func (uc *UserCreate) AddContactEntries(c ...*Contact) *UserCreate {
	ids := make([]int, len(c))
	for i := range c {
		ids[i] = c[i].ID
	}
	return uc.AddContactEntryIDs(ids...)
}
...
func (uc *UserCreate) AddInitiatedCallIDs(ids ...int) *UserCreate {
	uc.mutation.AddInitiatedCallIDs(ids...)
	return uc
}

// AddInitiatedCalls adds the "initiated_calls" edges to the CallLog entity.
func (uc *UserCreate) AddInitiatedCalls(c ...*CallLog) *UserCreate {
	ids := make([]int, len(c))
	for i := range c {
		ids[i] = c[i].ID
	}
	return uc.AddInitiatedCallIDs(ids...)
}

// AddCallParticipationIDs adds the "call_participations" edge to the CallParticipant entity by IDs.
func (uc *UserCreate) AddCallParticipationIDs(ids ...int) *UserCreate {
	uc.mutation.AddCallParticipationIDs(ids...)
	return uc
}

// AddCallParticipations adds the "call_participations" edges to the CallParticipant entity.
func (uc *UserCreate) AddCallParticipations(c ...*CallParticipant) *UserCreate {
	ids := make([]int, len(c))
	for i := range c {
		ids[i] = c[i].ID
	}
	return uc.AddCallParticipationIDs(ids...)
}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := uc.mutation.UploadedMediaIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: true,
	...
	if nodes := uc.mutation.OwnedRoomsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: true,
	...
	if nodes := uc.mutation.ContactEntriesIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: true,
	...
	if nodes := uc.mutation.InitiatedCallsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: true,
	...
	if nodes := uc.mutation.CallParticipationsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: true,
