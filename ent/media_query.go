...
func (mq *MediaQuery) QueryUploader() *UserQuery {
...
func (mq *MediaQuery) QueryMessage() *MessageQuery {
...
func (mq *MediaQuery) Clone() *MediaQuery {
	...
	return &MediaQuery{
		...
		withUploader: mq.withUploader.Clone(),
		withMessage:  mq.withMessage.Clone(),
...
func (mq *MediaQuery) WithUploader(opts ...func(*UserQuery)) *MediaQuery {
	query := (&UserClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	mq.withUploader = query
	return mq
}
...
func (mq *MediaQuery) WithMessage(opts ...func(*MessageQuery)) *MediaQuery {
	query := (&MessageClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	mq.withMessage = query
	return mq
}
...
func (mq *MediaQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Media, error) {
	...
	if mq.withUploader != nil || mq.withMessage != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := mq.withUploader; query != nil {
		if err := mq.loadUploader(ctx, query, nodes, nil,
			func(n *Media, e *User) { n.Edges.Uploader = e }); err != nil {
			return nil, err
		}
	}
	if query := mq.withMessage; query != nil {
		if err := mq.loadMessage(ctx, query, nodes, nil,
...
func (mq *MediaQuery) loadUploader(ctx context.Context, query *UserQuery, nodes []*Media, init func(*Media), assign func(*Media, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Media)
	for i := range nodes {
		if nodes[i].media_uploader == nil {
			continue
		}
		fk := *nodes[i].media_uploader
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "media_uploader" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (mq *MediaQuery) loadMessage(ctx context.Context, query *MessageQuery, nodes []*Media, init func(*Media), assign func(*Media, *Message)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Media)
	for i := range nodes {
		if nodes[i].media_message == nil {
			continue
		}
		fk := *nodes[i].media_message
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "media_message" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
