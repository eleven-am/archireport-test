...
func (mq *MessageQuery) QueryMedia() *MediaQuery {
...
func (mq *MessageQuery) WithMedia(opts ...func(*MediaQuery)) *MessageQuery {
	query := (&MediaClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (mq *MessageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Message, error) {
	...
	if mq.withSender != nil || mq.withRoom != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := mq.withMedia; query != nil {
		if err := mq.loadMedia(ctx, query, nodes,
			func(n *Message) { n.Edges.Media = []*Media{} },
			func(n *Message, e *Media) { n.Edges.Media = append(n.Edges.Media, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MessageQuery) loadSender(ctx context.Context, query *UserQuery, nodes []*Message, init func(*Message), assign func(*Message, *User)) error {
	...
	for i := range nodes {
		if nodes[i].message_sender == nil {
			continue
		}
		fk := *nodes[i].message_sender
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "message_sender" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (mq *MessageQuery) loadRoom(ctx context.Context, query *RoomQuery, nodes []*Message, init func(*Message), assign func(*Message, *Room)) error {
	...
	nodeids := make(map[int][]*Message)
	for i := range nodes {
		if nodes[i].message_room == nil {
			continue
		}
		fk := *nodes[i].message_room
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "message_room" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (mq *MessageQuery) loadMedia(ctx context.Context, query *MediaQuery, nodes []*Message, init func(*Message), assign func(*Message, *Media)) error {
	...
	for _, n := range neighbors {
		fk := n.media_message
		if fk == nil {
			return fmt.Errorf(`foreign-key "media_message" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "media_message" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
