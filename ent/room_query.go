...
func (rq *RoomQuery) QueryMemberships() *RoomMembershipQuery {
	query := (&RoomMembershipClient{config: rq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rq *RoomQuery) QueryFavourites() *FavouriteQuery {
	query := (&FavouriteClient{config: rq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rq *RoomQuery) QueryCallLogs() *CallLogQuery {
	query := (&CallLogClient{config: rq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rq *RoomQuery) Clone() *RoomQuery {
	...
	return &RoomQuery{
		...
		withFavourites:  rq.withFavourites.Clone(),
		withCallLogs:    rq.withCallLogs.Clone(),
...
func (rq *RoomQuery) WithMemberships(opts ...func(*RoomMembershipQuery)) *RoomQuery {
	query := (&RoomMembershipClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (rq *RoomQuery) WithFavourites(opts ...func(*FavouriteQuery)) *RoomQuery {
	query := (&FavouriteClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withFavourites = query
	return rq
}
...
func (rq *RoomQuery) WithCallLogs(opts ...func(*CallLogQuery)) *RoomQuery {
	query := (&CallLogClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withCallLogs = query
	return rq
}
...
func (rq *RoomQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Room, error) {
	...
	if query := rq.withMemberships; query != nil {
		if err := rq.loadMemberships(ctx, query, nodes,
			func(n *Room) { n.Edges.Memberships = []*RoomMembership{} },
			func(n *Room, e *RoomMembership) { n.Edges.Memberships = append(n.Edges.Memberships, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := rq.withFavourites; query != nil {
		if err := rq.loadFavourites(ctx, query, nodes,
			func(n *Room) { n.Edges.Favourites = []*Favourite{} },
			func(n *Room, e *Favourite) { n.Edges.Favourites = append(n.Edges.Favourites, e) }); err != nil {
			return nil, err
		}
	}
	if query := rq.withCallLogs; query != nil {
		if err := rq.loadCallLogs(ctx, query, nodes,
			func(n *Room) { n.Edges.CallLogs = []*CallLog{} },
			func(n *Room, e *CallLog) { n.Edges.CallLogs = append(n.Edges.CallLogs, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *RoomQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*Room, init func(*Room), assign func(*Room, *User)) error {
	...
	for i := range nodes {
		if nodes[i].room_owner == nil {
			continue
		}
		fk := *nodes[i].room_owner
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "room_owner" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (rq *RoomQuery) loadMemberships(ctx context.Context, query *RoomMembershipQuery, nodes []*Room, init func(*Room), assign func(*Room, *RoomMembership)) error {
	...
	query.Where(predicate.RoomMembership(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(room.MembershipsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.room_membership_room
		if fk == nil {
			return fmt.Errorf(`foreign-key "room_membership_room" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "room_membership_room" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (rq *RoomQuery) loadFavourites(ctx context.Context, query *FavouriteQuery, nodes []*Room, init func(*Room), assign func(*Room, *Favourite)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Room)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Favourite(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(room.FavouritesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.favourite_room
		if fk == nil {
			return fmt.Errorf(`foreign-key "favourite_room" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "favourite_room" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (rq *RoomQuery) loadCallLogs(ctx context.Context, query *CallLogQuery, nodes []*Room, init func(*Room), assign func(*Room, *CallLog)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Room)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.CallLog(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(room.CallLogsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.call_log_room
		if fk == nil {
			return fmt.Errorf(`foreign-key "call_log_room" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "call_log_room" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
